  ...
  handleKeyPress = event => {
    ...
    switch(event.keyCode) {
      case keyName.space:
        ...
        isPlaying ? this.handlePauseAudio() : this.handlePlayAudio();
      break;

      case keyName.arrowRight:
        event.preventDefault();
        newTime = window.audioInstance.currentTime + 10;

        if(duration > newTime) {
          this.handleChangeCurrentTime(newTime);  
        }
      break;
    ...
    if(event.keyCode === keyName.space) {
      event.preventDefault();
      if(isPlaying) this.handlePauseAudio();
      else this.handlePlayAudio();
    }
  }
